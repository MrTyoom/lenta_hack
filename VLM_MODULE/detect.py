...
# ============================================================
# SELECT CONFIGURATION HERE (change before running)
# Options: "4-bit NF4", "Fast-64", "Fast-128"
# ============================================================
...
# ============================================================
# PROMPT (English instruction + Russian output requirement)
# ============================================================
...
- layout_code: character Ш inside a circle (if present), otherwise leave empty
...
def parse_response(raw_text):
    ...
    if not fields['article']:
        article_match = re.search(r'(\d+_\d+)', cleaned)
        if article_match:
            fields['article'] = article_match.group(1)
    
    if not fields['barcode']:
        barcode_match = re.search(r'\b(\d{8,14})\b', cleaned)
        if barcode_match:
            fields['barcode'] = barcode_match.group(1)
    
    if not fields['discount_size']:
        discount_match = re.search(r'(-?\d+[%\s]*руб|рублей|\d+[%])', cleaned, re.IGNORECASE)
        if discount_match:
            fields['discount_size'] = discount_match.group(1)
...
def run_benchmark():
    ...
    if CURRENT_CONFIG not in configs:
        print(f"ERROR: Config '{CURRENT_CONFIG}' not found.")
        print(f"Available configs: {', '.join(configs.keys())}")
        return
    
    config_params = configs[CURRENT_CONFIG]
    ...
    for folder_name, images in folders.items():
        print(f"\n{'='*60}")
        print(f"FOLDER: {folder_name} ({len(images)} images)")
        print(f"{'='*60}")
        ...
        for idx, img_info in enumerate(images):
            ...
            print(f"  Processing [{idx+1}/{len(images)}]: {crop_name}")
            ...
            try:
                ...
                result_row = {
                    ...
                    'price_with_card': parsed['price_with_card'],
                    'promo_price': parsed['promo_price'],
                    'barcode': parsed['barcode'],
                    'discount_size': parsed['discount_size'],
                    'article': parsed['article'],
                    'layout_code': parsed['layout_code'],
                    'print_date': parsed['print_date'],
                    'raw_text': raw_response.replace('\n', ' <> ').strip()
                }
        ...
        folder_excel_path = os.path.join(folder_output_dir, f"{folder_name}.xlsx")
        folder_df.to_excel(folder_excel_path, index=False)
        print(f"  Saved: {folder_output_dir}/")
    ...
    print(f"All results saved to: {final_path}")
    
    model_dest = os.path.join(OUTPUT_DIR, "model")
    if not os.path.exists(model_dest):
        print(f"Copying model to: {model_dest}")
        shutil.copytree(MODEL_PATH, model_dest)
        print("Model copy complete.")
    else:
        print(f"Model already exists at: {model_dest}, skipping copy.")
    ...
    print(f"Total images processed: {len(all_df)}")
    
    valid = all_df[all_df['inference_time_sec'] > 0]
    if len(valid) > 0:
        avg_inf_time = valid['inference_time_sec'].mean()
        print(f"Avg inference time: {avg_inf_time:.2f}s")
